...
def test_api_cases_list_and_detail(tmp_path: Path) -> None:
    client = _build_client(tmp_path)

    response = client.get("/api/cases", params={"group": "OLL"})
    assert response.status_code == 200
    payload = response.json()
    assert payload["ok"] is True
    assert payload["data"]

    first_case_id = int(payload["data"][0]["id"])
    detail = client.get(f"/api/cases/{first_case_id}")
    assert detail.status_code == 200
    detail_payload = detail.json()["data"]
    assert detail_payload["id"] == first_case_id

    alternatives = client.get(f"/api/cases/{first_case_id}/alternatives")
    assert alternatives.status_code == 200
    alternatives_payload = alternatives.json()["data"]
    assert alternatives_payload
...
def test_api_progress_and_render_flow(tmp_path: Path) -> None:
    ...
    status_resp = client.get(f"/api/cases/{case_id}/renders/status")
    assert status_resp.status_code == 200
    status_payload = status_resp.json()["data"]
    assert isinstance(status_payload["jobs"], list)
    assert status_payload["algorithm_id"] == progress_resp.json()["data"]["active_algorithm_id"]


def test_api_alternatives_crud_flow(tmp_path: Path) -> None:
    client = _build_client(tmp_path)
    ...
    alternatives_resp = client.get(f"/api/cases/{case_id}/alternatives")
    assert alternatives_resp.status_code == 200
    alternatives = alternatives_resp.json()["data"]
    assert any(item["id"] == new_active_id and item["is_active"] for item in alternatives)

    activate_resp = client.post(
        f"/api/cases/{case_id}/active-algorithm",
    ...
    delete_resp = client.delete(f"/api/cases/{case_id}/alternatives/{new_active_id}")
