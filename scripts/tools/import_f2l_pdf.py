...
DEFAULT_SEED_PATH = REPO_ROOT / "db" / "cards_seed.sql"
...
def _generate_seed_sql(cases: list[dict]) -> str:
    lines: list[str] = []
    lines.append(BEGIN_MARKER)
    lines.append("-- Auto-generated by scripts/tools/import_f2l_pdf.py from data/f2l/best_f2l_from_pdf.yaml")
    lines.append("DELETE FROM canonical_algorithms")
    lines.append("WHERE canonical_case_id IN (")
    lines.append("  SELECT id FROM canonical_cases WHERE category_code = 'F2L'")
    lines.append(");")
    lines.append("DELETE FROM canonical_cases WHERE category_code = 'F2L';")
    ...
    for case in cases:
        ...
        lines.append(
            ...
            f"VALUES ('F2L', '{_sql_escape(case_code)}', '{_sql_escape(title)}', '{_sql_escape(subgroup)}', {case_number}, NULL, 'F', 0, {sort_order}) "
        ...
        algorithms = list(case.get("algorithms") or [])
        for algo_index, algo in enumerate(algorithms):
            name = str(algo.get("name") or f"Algo {algo_index + 1}")
            formula = str(algo.get("formula") or "")
            primary = 1 if bool(algo.get("primary")) else 0
            sort = algo_index + 1
            lines.append(
                "INSERT INTO canonical_algorithms (canonical_case_id, name, formula, is_primary, sort_order) "
                f"SELECT id, '{_sql_escape(name)}', '{_sql_escape(formula)}', {primary}, {sort} "
                "FROM canonical_cases WHERE category_code='F2L' AND case_code='{case_code}' "
                "ON CONFLICT(canonical_case_id, name) DO UPDATE SET "
                "formula=excluded.formula, is_primary=excluded.is_primary, sort_order=excluded.sort_order;".format(
                    case_code=_sql_escape(case_code)
                )
            )
...
def validate_yaml_payload(payload: dict) -> None:
    if int(payload.get("version", 0)) != 1:
        raise ValueError("YAML payload must have version=1")
    cases = payload.get("cases")
    if not isinstance(cases, list) or not cases:
        raise ValueError("YAML payload must contain non-empty cases list")

    seen_pairs: set[tuple[str, str]] = set()
    for case in cases:
        subgroup = str(case.get("subgroup") or "").strip()
        code = str(case.get("case_code") or "").strip()
        if not subgroup:
            raise ValueError("Case subgroup is required")
        if not code:
            raise ValueError("Case case_code is required")

        key = (subgroup, code)
        if key in seen_pairs:
            raise ValueError(f"Duplicate case detected: subgroup={subgroup}, case_code={code}")
        seen_pairs.add(key)

        algos = case.get("algorithms")
        if not isinstance(algos, list) or not algos:
            raise ValueError(f"Case {code} must include at least one algorithm")

        primary_count = sum(1 for algo in algos if bool(algo.get("primary")))
        if primary_count != 1:
            raise ValueError(f"Case {code} must have exactly one primary algorithm (got {primary_count})")

        for algo in algos:
            formula = _normalize_formula(str(algo.get("formula") or ""))
            if not formula:
                raise ValueError(f"Case {code} has empty formula")
            try:
                FormulaConverter.convert_steps(formula, repeat=1)
            except Exception as exc:
                raise ValueError(f"Invalid formula for case {code}: {formula}") from exc
...
def main() -> int:
    ...
    parser.add_argument("--sync-seed", action="store_true", help="Generate and write db/cards_seed.sql F2L block from YAML payload.")
    ...
    if args.sync_seed:
        ...
        block = _generate_seed_sql(cases)
