...
class CubeMoveExtended(Animation):
    ...
    def __init__(self, mobject: RubikCube3D, move: str, **kwargs):
        self.move = move
    ...
    @staticmethod
    def targets_for_base(cube: RubikCube3D, base: str):
        dim = cube.dimensions
        mid = dim // 2
...
class CubeMoveConcurrent(Animation):
    ...
    def __init__(self, mobject: RubikCube3D, moves: list[str], **kwargs):
        if len(moves) < 2:
