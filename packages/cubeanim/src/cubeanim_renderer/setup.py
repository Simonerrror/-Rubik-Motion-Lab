...
class SceneSetup:
    @staticmethod
    def _soften_internal_faces(cube: RubikCube3D, internal_face_color: str) -> None:
    ...
    @staticmethod
    def apply(scene: ThreeDScene, config: CubeVisualConfig) -> RubikCube3D:
        scene.camera.background_color = config.background_color

        validate_cube_palette(config.colors)
        cube = RubikCube3D(colors=tuple(config.colors)).scale(config.scale)
