...
from cubeanim_renderer.render_base import (
    BaseRenderer,
    ManimRenderer,
...
def normalize_group(raw_group: str | object | None) -> str:
...
def _catalog_path(repo_root: Path) -> Path:
    return _workspace_root(repo_root) / "render_catalog.json"
...
def _next_alternative_name(
    ...
    for folder in _quality_folders(quality):
        media_dir = _workspace_root(repo_root) / "videos" / group / folder
        if media_dir.exists():
...
def _build_final_path(repo_root: Path, group: str, quality: str, output_name: str) -> Path:
    return _workspace_root(repo_root) / "videos" / group / quality / f"{output_name}.mp4"
