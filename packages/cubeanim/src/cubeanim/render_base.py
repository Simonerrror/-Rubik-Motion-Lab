...
class ManimRenderer(BaseRenderer):
    def build_command(
        ...
        for item in (repo_root / "packages" / "cubeanim" / "src", repo_root):
            token = str(item)
            if token not in pythonpath_parts:
                pythonpath_parts.append(token)
        existing_pythonpath = env.get("PYTHONPATH", "").strip()
        if existing_pythonpath:
            for token in existing_pythonpath.split(os.pathsep):
                cleaned = token.strip()
                if cleaned and cleaned not in pythonpath_parts:
                    pythonpath_parts.append(cleaned)
